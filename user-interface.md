...
توانایی خواندن اطلاعات از ورودی استاندارد و‫ ارسال آن به خروجی استاندارد با یک ویژگی شل (Shell) به نام پایپ لاین در خط فرمان  یا همان کاراکتر خط عمودی | ایجاد شده است . با استفاده از عملگر خط عمودی | خروجی استاندارد یک دستور به داخل ورودی استاندارد دستوری دیگر ریخته می شود. با اتصال دستورها به یکدیگر می توانید شاهکارهای شگفت انگیزی انجام دهید.‬
```
$ ls -l | less
```
...
‫بش فایل یک فایل متنی است که شامل یکسری دستورالعمل های خاص است.که هدفی را دنبال میکند.با دستور echo شروع شده و با Exit پایان می یابد.این فایل با پسوند bat. ذخیره می‌شود.‬
```
